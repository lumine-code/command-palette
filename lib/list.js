...
class CommandPalette {
  constructor(recentlyUsed) {
    ...
    this.configObserver = lumine.config.onDidChange("command-palette.recentCount", ({ newValue }) =>
      this.selectListView.setRecentLimit(newValue),
    );
  }
  ...
  toggleHiddenCommands() {
    ...
    return this.selectListView.reload();
  }
  ...
  toggleDescriptions() {
    ...
    return this.selectListView.refresh();
  }

  toggle() {
    if (this.selectListView.isVisible()) return this.selectListView.hide();
    return this.show();
  }
