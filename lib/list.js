...
class CommandPalette {
  constructor(recentlyUsed) {
    ...
    // Descriptions are on by default and the toggle is for the open palette
    // only, so every fresh show starts from the full two-line rows.
    ...
    this.commandsDisposable = lumine.commands.add(this.selectListView.element, {
      ...
      "command-palette:toggle-descriptions": {
        description: "Hide the command descriptions and leave them out of the search.",
        actionScope: "list",
  ...
  // reset marks the list itself. Without this a palette reopened after hiding
  // the descriptions renders them again while still filtering without them.
