...
class CommandPalette {
  constructor(recentlyUsed) {
    ...
    this.selectListView = lumine.workspace.buildSelectList({
      ...
      confirmAction: "command-palette:run-selected-command",
      additionalActionCommands: ["command-palette:clear-recent"],
      order: (a, b) =>
      ...
      willShow: () => {
        // `show()` and `toggle()` capture the command target before the modal
        // takes focus. Do not recapture it here: returning from Actions also
        // runs willShow, while focus still belongs to that child dialog.
      ...
      didConfirmSelection: (item) => this.runSelectedCommand(item),
      ...
      actionsFilter: ({ name }) => {
        if (name === "command-palette:clear-recent") return this.recentlyUsed.length > 0;
        return (
          name !== "command-palette:remove-from-recent" ||
          this.isRecent(this.selectListView.getSelectedItem())
        );
      },
    });
    ...
    this.commandsDisposable = lumine.commands.add(this.selectListView.element, {
      "command-palette:run-selected-command": {
        description: "Run the selected command on the surface that opened the palette.",
        didDispatch: () => this.runSelectedCommand(),
      },
      "command-palette:remove-from-recent": {
        description: "Drop the selected command from the recent section, leaving the list open.",
  ...
  runSelectedCommand(item = this.selectListView.getSelectedItem()) {
    if (!item) return;
    this.selectListView.hide();
    this.recordRecent(item);
    return lumine.commands.dispatch(this.activeElement, item.name);
  }
  ...
  toggle() {
    ...
    if (!this.selectListView.isVisible()) this.captureActiveElement();
  ...
  captureActiveElement() {
    // Capture the dispatch context before the select list redirects focus to
    // the primary window. The palette is primary chrome, but an element-local
    // command must still run on the detached element that opened it.
    this.activeSurface = lumine.workspace.getActiveWindowSurface();
    const targetDocument = this.activeSurface.document;
    const focused = lumine.dom.activeElementFor(this.activeSurface);
    this.activeElement =
      focused && focused !== targetDocument.body
        ? focused
        : this.activeSurface.element || targetDocument.querySelector("lumine-workspace");
  }

  isRecent(item) {
    return Boolean(item?.name) && this.recentlyUsed.includes(item.name);
  }
  ...
  // Forgetting one command is something a user does to several in a row, so
  // the list stays open and the row keeps its selection — it only moves out
  // of the section, down to where its own name puts it.
  removeFromRecent() {
    const item = this.selectListView.getSelectedItem();
    if (!this.isRecent(item)) return;
    const name = item.name;
    this.recentlyUsed.splice(this.recentlyUsed.indexOf(name), 1);
    this.needsUpdate = true;

    if (this.selectListView.isVisible?.()) {
      this.selectListView.update(this.listProps());
      // The rows are rebuilt from the registry, so the row to keep selected is
      // found again by name rather than by the object the action started from.
      const row = this.selectListView.items?.find((each) => each.name === name);
      if (row) this.selectListView.selectItem(row);
    }
  }
